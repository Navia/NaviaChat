...
public class NaviaChatDataSource {
	public static NaviaChat plugin;
	...
	public NaviaChatDataSource(NaviaChat instance){
	...
	/*public static boolean chTableExists(){
		...
		try {
			...
			log.log(Level.SEVERE, "[NaviaChat] Exception while checking table.", e);
			return false;
		} finally {
			try {
				if (rs != null){
				...
				Logger log = Logger.getLogger("Minecraft");
				log.log(Level.SEVERE, "[NaviaChat] Exception on table check close.", e);
			}
		}
	}
	...
	public static void createChTable(){
		...
		try {
			...
			Logger log = Logger.getLogger("Minecraft");
			log.log(Level.SEVERE, "[NaviaChat] Exception while creating table.", e);
			...
			try {
				...
				Logger log = Logger.getLogger("Minecraft");
				log.log(Level.SEVERE, "[NaviaChat] Could not create table. Exception on close.", e);
			}
		}
	}
	...
	public static void addChannel(Channel channel){
		...
		try{
            ...
            try {
                ...
                log.log(Level.SEVERE, "[NaviaChat] Channel Insert Exception (on close)", ex);
            }
        }
	}
	...
	public static void removeChannel(Channel channel){
		...
		try{
            ...
            log.log(Level.SEVERE, "[NaviaChat] Channel deleting exception", ex);
        } finally {
            try {
                if (ps != null) {
                    ps.close();
                ...
                log.log(Level.SEVERE, "[NaviaChat] Exception while deleting channel. (on close)", ex);
            }
        }
	}
	...
	public static List<Channel> getChannels(){
		...
		try {
			...
			log.info("[NaviaChat] " + size + " channels loaded.");
		} catch (SQLException e){
			log.log(Level.SEVERE, "[NaviaChat] Exception while loading table.", e);
		} finally {
			try {
				if (st != null){
					st.close();
				...
				log.log(Level.SEVERE, "[NaviaChat] Channels loading exception, while closing.", e);
			}
		}
		return channels;
	}*/
	...
	public static Channel convertPropertyToChannel(File iPropertyfile){
		// Trying out a new way of flatfile loading.
		...
		ArrayList<Player> banned = null; 
		if (prop.keyExists("banned-players")){
			banned = new ArrayList<Player>();
			String unFormatted = prop.getString("banned-players");
		...
		Channel c;
		if (banned == null){
			c = new Channel(plugin, range, name, sCut, color, ic, join, focus);
		} else {
			c = new Channel(plugin, range, name, sCut, color, ic, join, focus);
		}*/
		Logger log = Logger.getLogger("Minecraft");
		Channel c = new Channel(plugin, prop);
		c.initialize();
		
		log.info("[NaviaChat] Loaded channel: " + iPropertyfile.getName());
		
		return c;
	}
	
	public static void banPlayer(String player, Channel channel){
