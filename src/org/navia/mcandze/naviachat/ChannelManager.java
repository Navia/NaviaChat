...
public class ChannelManager {
	...
	public ChannelManager(NaviaChat instance, NaviaChatDataSource dsInstance){
	...
	public void playerLeaveChannel(String channel, Player player){
		...
		Channel ch;
		if ((ch = getChannelWithShortcut(channel)) == null){
			return;
		}
		playerChannels.get(player).remove(ch);
	...
	public void loadChannels(){
		...
		for (Channel c: plugin.getDataSource().loadChannels()){
			channels.add(c);
		}
		int size = 0;
		for (Channel c: channels){
		...
		log.info("[NaviaChat] Loaded " + size + " channels.");
	}
